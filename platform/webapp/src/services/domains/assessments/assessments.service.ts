...
const rawAssessmentsService = {
   ...
   */
  async getAssessment(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/assessment` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createAssessment(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/assessment`;
   ...
   */
  async getExemption(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/exemptions` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async applyExemption(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/exemptions`;
