 ...
 *
 * API client for correspondence domain.
...
const rawCorrespondenceService = {
  /**
   * List correspondence for a case
   */
  async getCorrespondence(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/correspondence` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createCorrespondence(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/correspondence`;
...
export const correspondenceService = makeService(rawCorrespondenceService, "correspondence");
