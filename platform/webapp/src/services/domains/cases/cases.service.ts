...
const rawCasesService = {
  /**
   * List erasure cases (desk filters)
   */
  async listErasureCases(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createErasureCase(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases`;
   ...
   */
  async getErasureCase(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createVerbal(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/intake/verbal`;
   ...
   */
  async getIntakeEvent(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/erasure-cases/${caseId}/intake-events` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const casesService = makeService(rawCasesService, "cases");
